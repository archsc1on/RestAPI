...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('facebook.com') && !url.includes('fb.watch')) {
      return NextResponse.json({ status: false, message: 'Invalid Facebook URL' }, { status: 400 })
    }
    ...
    if (result.status === 'picker' && result.picker) {
      await prisma.apiLog.create({ data: { keyId: keyRecord.id, userId: keyRecord.userId, endpoint: '/api/tools/fbdl', method: 'GET', statusCode: 200, costCredits: creditCost, ip: request.headers.get('x-forwarded-for') || '', userAgent: request.headers.get('user-agent') || '' } })
      ...
      return NextResponse.json({
        status: true,
        data: { type: 'facebook', originalUrl: url, items: result.picker.map((p) => ({ type: p.type, url: p.url })), costCredits: creditCost, remaining: user?.credits || 0 }
      })
    }
    ...
    return NextResponse.json({
      status: true,
      data: { type: 'facebook', originalUrl: url, downloadUrl: result.url, filename: result.filename, costCredits: creditCost, remaining: user?.credits || 0 }
    })
  } catch (error: any) {
