...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('vimeo.com')) return NextResponse.json({ status: false, message: 'Invalid Vimeo URL' }, { status: 400 })
    ...
    console.error('[vimeo-dl] error:', error?.message || error)
