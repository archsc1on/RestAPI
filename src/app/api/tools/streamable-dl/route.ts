...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('streamable.com')) return NextResponse.json({ status: false, message: 'Invalid Streamable URL' }, { status: 400 })
    ...
    if (result.status === 'tunnel' || result.status === 'redirect') {
      ...
      filename = filename || result.output?.filename || 'streamable_download'
    } else if (result.status === 'picker' && result.picker && result.picker.length > 0) {
      downloadUrl = result.picker[0].url
      filename = filename || 'streamable_download'
    }

    await prisma.apiLog.create({
      data: {
        keyId: keyRecord.id, userId: keyRecord.userId,
        endpoint: '/api/tools/streamable-dl', method: 'GET', statusCode: 200,
    ...
    console.error('[streamable-dl] error:', error?.message || error)
