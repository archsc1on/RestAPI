...
export async function GET(request: NextRequest) {
  try {
    ...
    // Handle picker (carousel posts)
    if (result.status === 'picker' && result.picker) {
      await prisma.apiLog.create({ data: { keyId: keyRecord.id, userId: keyRecord.userId, endpoint: '/api/tools/igdl', method: 'GET', statusCode: 200, costCredits: creditCost, ip: request.headers.get('x-forwarded-for') || '', userAgent: request.headers.get('user-agent') || '' } })
    ...
    return NextResponse.json({ status: false, message: error?.message || 'Failed to process media' }, { status: 500 })
  }
}
