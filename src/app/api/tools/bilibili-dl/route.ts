...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('bilibili.com') && !url.includes('b23.tv')) {
      return NextResponse.json({ status: false, message: 'Invalid Bilibili URL' }, { status: 400 })
    }
    ...
    if (result.status === 'tunnel' || result.status === 'redirect') {
      ...
      filename = filename || result.output?.filename || 'bilibili_download'
    } else if (result.status === 'picker' && result.picker && result.picker.length > 0) {
      downloadUrl = result.picker[0].url
      filename = filename || 'bilibili_download'
    }

    await prisma.apiLog.create({
      data: {
        keyId: keyRecord.id, userId: keyRecord.userId,
        endpoint: '/api/tools/bilibili-dl', method: 'GET', statusCode: 200,
    ...
    console.error('[bilibili-dl] error:', error?.message || error)
