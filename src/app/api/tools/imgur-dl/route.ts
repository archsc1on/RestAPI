...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('imgur.com')) return NextResponse.json({ status: false, message: 'Invalid Imgur URL' }, { status: 400 })
    ...
    console.error('[imgur-dl] error:', error?.message || error)
