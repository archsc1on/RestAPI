...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('soundcloud.com')) return NextResponse.json({ status: false, message: 'Invalid SoundCloud URL' }, { status: 400 })
    ...
    if (result.status === 'tunnel' || result.status === 'redirect') {
      ...
      filename = filename || result.output?.filename || 'soundcloud_download'
    } else if (result.status === 'picker' && result.picker && result.picker.length > 0) {
      downloadUrl = result.picker[0].url
      filename = filename || 'soundcloud_download'
    }

    await prisma.apiLog.create({
      data: {
        keyId: keyRecord.id, userId: keyRecord.userId,
        endpoint: '/api/tools/soundcloud-dl', method: 'GET', statusCode: 200,
    ...
    console.error('[soundcloud-dl] error:', error?.message || error)
