...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('dailymotion.com')) return NextResponse.json({ status: false, message: 'Invalid Dailymotion URL' }, { status: 400 })
    ...
    if (result.status === 'tunnel' || result.status === 'redirect') {
      ...
      filename = filename || result.output?.filename || 'dailymotion_download'
    } else if (result.status === 'picker' && result.picker && result.picker.length > 0) {
      downloadUrl = result.picker[0].url
      filename = filename || 'dailymotion_download'
    }

    await prisma.apiLog.create({
      data: {
        keyId: keyRecord.id, userId: keyRecord.userId,
        endpoint: '/api/tools/dailymotion-dl', method: 'GET', statusCode: 200,
    ...
    console.error('[dailymotion-dl] error:', error?.message || error)
