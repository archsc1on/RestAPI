...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('twitch.tv')) {
      return NextResponse.json({ status: false, message: 'Invalid Twitch URL' }, { status: 400 })
    }
    ...
    if (result.status === 'tunnel' || result.status === 'redirect') {
      ...
      filename = filename || result.output?.filename || 'twitch_download'
    } else if (result.status === 'picker' && result.picker && result.picker.length > 0) {
      downloadUrl = result.picker[0].url
      filename = filename || 'twitch_download'
    }

    await prisma.apiLog.create({
      data: {
        keyId: keyRecord.id, userId: keyRecord.userId,
        endpoint: '/api/tools/twitch-dl', method: 'GET', statusCode: 200,
    ...
    console.error('[twitch-dl] error:', error?.message || error)
