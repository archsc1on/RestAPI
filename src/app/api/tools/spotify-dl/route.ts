...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!url.includes('spotify.com')) return NextResponse.json({ status: false, message: 'Invalid Spotify URL' }, { status: 400 })

    const result = await cobaltDownload({ url, downloadMode: 'audio', audioFormat: 'mp3' })

    if (result.status === 'error') {
      return NextResponse.json({ status: false, message: result.error?.context || result.error?.code || 'Spotify content may be DRM-protected. Download failed.' }, { status: 502 })
    }
    ...
    if (result.status === 'tunnel' || result.status === 'redirect') {
      ...
      filename = filename || result.output?.filename || 'spotify_download'
    } else if (result.status === 'picker' && result.picker && result.picker.length > 0) {
      downloadUrl = result.picker[0].url
      filename = filename || 'spotify_download'
    }

    await prisma.apiLog.create({
      data: {
        keyId: keyRecord.id, userId: keyRecord.userId,
        endpoint: '/api/tools/spotify-dl', method: 'GET', statusCode: 200,
    ...
    console.error('[spotify-dl] error:', error?.message || error)
